"""
Method 1: Baseline — Synchronous Eager FP32 Execution
Every operation dispatched sequentially. H2D transfer blocks the CPU thread until
the DMA confirms completion. GPU sits idle during each H2D window (~7.3ms at 1024x1024,
much shorter at 256x256 due to smaller batch volume).
4 fan-in sync barriers per forward pass at torch.cat() skip connections.
"""
import os
import time
import json
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for inputs_cpu, targets_cpu in train_loader:
            ...
            # --- Measure synchronous H2D transfer ---
            ...
            # --- Measure forward + backward + optimizer ---
        ...
        # Baseline: sequential, so step = H2D + compute

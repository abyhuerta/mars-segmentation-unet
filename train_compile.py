"""
Method 3: torch.compile (Kernel Fusion via TorchDynamo + TorchInductor)
Captures the full forward graph, fuses Conv->BN->ReLU chains into single kernels,
eliminating intermediate VRAM reads/writes. Cannot fuse across skip connection
boundaries (encoder tensors must be materialized to survive bottleneck compute).
Compilation cost: ~60-120s first run, cached to disk for subsequent runs.
Breakeven: ~3.6 epochs at this step size.
"""
import os
import time
import json
...
def main():
    ...
    # --- torch.compile: TorchDynamo graph capture + kernel fusion via TorchInductor ---
    ...
    # Warmup: forward + backward triggers actual compilation (lazy on first call)
    print("  Warming up (triggers compilation on first forward+backward pass)...")

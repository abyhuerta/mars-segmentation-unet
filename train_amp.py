"""
Method 2: Automatic Mixed Precision (AMP)
Routes Conv2d/Linear ops to Tensor Cores (FP16), keeps BatchNorm/loss in FP32.
GradScaler prevents gradient underflow. Adds 4 dtype-cast events at skip connections
plus 1 D2H GradScaler overflow check per step (9 total non-compute sync events).
"""
import os
import time
import json
...
def run_validation(model, val_loader, criterion, device):
    ...
    avg_miou = val_miou_total / num_batches if num_batches > 0 else 0.0
    avg_class_ious = [t / num_batches for t in class_ious_totals] if num_batches > 0 else [0.0] * NUM_CLASSES
    return avg_val_loss, avg_miou, avg_class_ious
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for inputs_cpu, targets_cpu in train_loader:
            ...
            # --- Measure compute time (forward + backward + optimizer) ---
        ...
        # Full step = H2D + compute (sequential in AMP, no overlap)
